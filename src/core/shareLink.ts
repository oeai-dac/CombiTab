 ...
 *
 * Kodiert das vollständige Projekt (inkl. Seriationsreihenfolge, Filter, View) plus
 * etwas UI-Zustand in den URL-Fragment. Komprimiert per nativem `CompressionStream`
...
export async function decodeShare(fragment: string): Promise<ShareState | null> {
  try {
    ...
    return s && s.project && Array.isArray(s.project.contexts) ? s : null;
  } catch { return null; }
}
