 ...
 * Matrix-Renderer-Farben aus den CSS-Tokens aus (`--mx-*`), damit die
 * WebGL-Matrix beim Umschalten mitzieht. Framework-frei.
 */
import type { MatrixTheme } from "../matrix/MatrixRenderer.js";

export type Theme = "light" | "dark";
const KEY = "combitab.theme";
export const THEME_EVENT = "combitab:theme";
...
/** Liest die aktuellen Renderer-Farben aus den CSS-Tokens des `<html>`. */
export function readMatrixTheme(): MatrixTheme {
  const cs = getComputedStyle(document.documentElement);
  const v = (name: string, fallback: string) => (cs.getPropertyValue(name).trim() || fallback);
  return {
    bg: toRgb01(v("--mx-bg", "#f6f4f2")),
    label: v("--mx-label", "#5d584f"),
    labelDim: v("--mx-label-dim", "#8b857c"),
    labelActive: v("--mx-label-active", "#a81d26"),
  };
}

/** Neutrale Farben für die kanvas-gezeichneten Analyse-Plots (CA, Ford, Stabilität). */
