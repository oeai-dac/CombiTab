# CombiTab installieren

Diese Anleitung führt Schritt für Schritt durch die Installation. Sie setzt
keine technischen Vorkenntnisse voraus.

> **Ganz ohne Installation:** CombiTab läuft auch direkt im Browser unter
> **<https://oeai-dac.github.io/CombiTab/>**. Alle Berechnungen finden dabei auf
...
[**Releases**](https://github.com/oeai-dac/CombiTab/releases) unter dem
...
CombiTab wird als freie Software eines Forschungsinstituts herausgegeben und ist
...
[Actions](https://github.com/oeai-dac/CombiTab/actions).
...
1. Laden Sie `CombiTab-2.0.0-Setup-x64.exe` herunter.
...
5. CombiTab liegt anschließend im Startmenü und auf dem Desktop.
...
Laden Sie `CombiTab-2.0.0-portable-x64.exe` herunter und doppelklicken Sie sie.
...
5. **Wichtig beim ersten Start:** Doppelklicken Sie CombiTab **nicht**, sondern
...
### Falls „CombiTab ist beschädigt und kann nicht geöffnet werden" erscheint
...
2. Tippen Sie folgende Zeile ein und drücken Sie Enter:
   ...
   ```
   xattr -cr /Applications/CombiTab.app
   ```

3. Starten Sie CombiTab nun normal.
...
1. Laden Sie `CombiTab-2.0.0-x86_64.AppImage` herunter.
2. Machen Sie die Datei ausführbar. Entweder im Dateimanager
   ...
   ```bash
   chmod +x CombiTab-2.0.0-x86_64.AppImage
   ```

3. Doppelklicken Sie die Datei, oder starten Sie sie im Terminal:

   ```bash
   ./CombiTab-2.0.0-x86_64.AppImage
   ```

Möchten Sie CombiTab im Anwendungsmenü sehen, hilft das Werkzeug
...
```bash
sudo dnf install ./combitab-2.0.0.x86_64.rpm     # Fedora, RHEL
sudo zypper install ./combitab-2.0.0.x86_64.rpm  # openSUSE
```

Nach der Installation erscheint CombiTab im Anwendungsmenü unter
...
Beim Start lädt CombiTab einen Beispieldatensatz, an dem Sie alle Funktionen
...
- CombiTab-v2-Projekte (`.combitab.json`)
- Projekte aus CombiTab Version 1 (`.json`) — sie werden automatisch übernommen

---

## Datenschutz

CombiTab arbeitet vollständig auf Ihrem Rechner. Es gibt keinen Server, keine
...
| System | Vorgehen |
|---|---|
| Windows | Einstellungen → Apps → CombiTab → Deinstallieren |
| Windows (portabel) | Die `.exe`-Datei löschen |
| macOS | `CombiTab.app` aus „Programme" in den Papierkorb ziehen |
| Linux (AppImage) | Die `.AppImage`-Datei löschen |
| Debian/Ubuntu | `sudo apt remove combitab` |
| Fedora/RHEL | `sudo dnf remove combitab` |
