/**
 * CombiTab v2 — Electron-Hauptprozess.
 *
 ...
 * blockiert die unter file:// über die CORS-Regeln, womit Korrespondenzanalyse,
 * Bootstrap und Score-Berechnung ausfielen. Ein eigenes Schema liefert einen
...
const SMOKE = process.env.COMBITAB_SMOKE === "1";

const APP_SCHEME = "app";
const APP_ORIGIN = `${APP_SCHEME}://combitab`;
...
/** Bedient app://combitab/… aus dem dist-Verzeichnis, mit Schutz vor Pfadausbruch. */
...
function handleDownloads(sess) {
  sess.on("will-download", (_event, item) => {
    ...
    const known = {
      ...
      csv: "CSV-Tabelle", xlsx: "Excel-Arbeitsmappe", json: "CombiTab-Projekt",
      md: "Markdown-Bericht", ttl: "Turtle (RDF)", jsonld: "JSON-LD",
    };
...
function buildMenu(win) {
  ...
  const about = () => {
    dialog.showMessageBox(win, {
      type: "info",
      title: t("Über CombiTab", "About CombiTab"),
      message: `CombiTab ${app.getVersion()}`,
      detail: t(
        "Kombinationstabellen, Seriation und Korrespondenzanalyse für die\n" +
        ...
        "Combination tables, seriation and correspondence analysis for\n" +
  ...
  const template = [
    ...(isMac
      ? [{
          label: app.name,
          submenu: [
            { label: t("Über CombiTab", "About CombiTab"), click: about },
            { type: "separator" },
            { role: "services" },
            { type: "separator" },
            { role: "hide", label: t("CombiTab ausblenden", "Hide CombiTab") },
            { role: "hideOthers", label: t("Andere ausblenden", "Hide Others") },
            { role: "unhide", label: t("Alle einblenden", "Show All") },
            { type: "separator" },
            { role: "quit", label: t("CombiTab beenden", "Quit CombiTab") },
          ],
        }]
      : []),
    {
      ...
      submenu: [
        {
          ...
          click: () => shell.openExternal("https://github.com/oeai-dac/CombiTab"),
        },
        {
          label: t("Fehler melden", "Report an issue"),
          click: () => shell.openExternal("https://github.com/oeai-dac/CombiTab/issues"),
        },
        ...(isMac ? [] : [{ type: "separator" }, { label: t("Über CombiTab", "About CombiTab"), click: about }]),
      ],
    },
  ];
